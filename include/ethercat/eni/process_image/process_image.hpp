...
ProcessImage::VariablesList ProcessImage::get_slave_variables(Direction direction, std::string_view name) const {
    return get_variables(direction).get_slave_variables(name);
}
