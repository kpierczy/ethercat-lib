...
class Master : public ::ethercat::drivers::Master<Master> {

    /// Make base class a friend to let it access implementation methods
    friend class ::ethercat::drivers::Master<Master>;
...
protected: /* -------------------------------- Protected EtherCAT I/O methods (implementation) ------------------------------------ */
     ...
     *    on failure
     */
    void read_bus_impl(ranges::span<uint8_t> pdi_buffer);
     ...
     *    on failure
     */
    void write_bus_impl(ranges::span<uint8_t> pdi_buffer);
